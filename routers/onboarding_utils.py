...
def _build_system_prompt(
    ...
    return (
        ...
        f"{hint_block}"
        f"\n=== КАРТА ОНБОРДИНГА ===\n"
        f"Ты ведёшь онбординг. Вот все шаги по порядку:\n"
        f"1.имя владельца → 2.кличка питомца → 3.цель → 4.вид(кошка/собака) → "
        f"5.паспорт → 6.порода → 7.дата рождения → 8.пол → 9.кастрация → 10.фото → ФИНАЛ\n"
        f"Текущий шаг: {current_step}\n"
        f"Ты задаёшь ТОЛЬКО вопрос текущего шага. Вопросы других шагов ЗАПРЕЩЕНЫ.\n"
        f"=========================\n"
        f"\n=== ЖЕЛЕЗНЫЕ ПРАВИЛА ===\n"
