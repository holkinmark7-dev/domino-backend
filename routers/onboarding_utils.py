...
def _validate_input_with_ai(text: str, field: str, collected: dict) -> dict:
    ...
    prompts = {
        "owner_name": (
            ...
            f'Он написал: "{text}"\n\n'
            f'ПРАВИЛА:\n'
            f'ПРИНЦИП: Считай что человек ХОЧЕТ назвать имя но говорит по-своему.\n'
            f'Твоя задача — найти имя если оно есть, а не повод отклонить.\n'
            f'Если имя есть в тексте среди других слов — извлеки его.\n'
            f'Если имени нет — отклони.\n\n'
...
def _build_system_prompt(
    ...
    return (
        ...
        f"{hint_block}"
        f"\n=== ЖЕЛЕЗНЫЕ ПРАВИЛА ===\n"
        f"1. Следуй инструкции в задаче. Если указан точный текст — скажи его без изменений. Если указана ЦЕЛЬ — действуй свободно в рамках цели.\n"
