...
def _get_step_instruction(step: str, collected: dict) -> str:
    ...
    if step == "species":
        ...
        return (
            ...
            f"Коротко — один вопрос, без дополнительных тем.\n"
    ...
    if step == "passport_offer":
        return (
            ...
            f"Зачем: ты сам перенесёшь все данные из паспорта в карточку. Экономит время — не нужно вводить породу, дату рождения, прививки вручную.\n"
            f"1-2 предложения.\n"
    ...
    if step == "avatar":
        return (
            ...
            f"Фото станет аватаркой в карточке {pet_gen} — как фото профиля.\n"
            f"Это последний шаг знакомства.\n"
            f"1-2 предложения. Тепло, легко.\n"
            f"РАМКИ: не спрашивай про другое. Не придумывай другие причины зачем нужно фото."
        )
